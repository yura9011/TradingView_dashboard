...
Este tutorial te guiará paso a paso para configurar y ejecutar el sistema de análisis de trading usando el modelo local **Qwen2-VL-7B-Instruct**, sin necesidad de APIs externas.
...
| Almacenamiento | 30 GB libres | 100 GB SSD |
| CPU | 4 cores | 8+ cores |

### GPUs Compatibles
- **NVIDIA**: RTX 3060 (12GB), RTX 3080, RTX 4070, RTX 4080, RTX 4090
- **AMD**: ROCm compatible (experimental)
- **CPU**: Funciona pero es muy lento (10-30 minutos por análisis)

### Software Requerido
- **Python**: 3.10 o 3.12 (recomendado)
...
⚠️ La primera ejecución descargará el modelo (~15GB). Esto puede tomar tiempo dependiendo de tu conexión.
...
**Causa**: GPU sin suficiente VRAM. Qwen2-VL-7B requiere más memoria que Phi-3.5.
...
2. Asegúrate de tener 30GB+ libres
...
El sistema está optimizado para `Qwen/Qwen2-VL-7B-Instruct`. Otros modelos pueden requerir cambios en el código (procesamiento de imágenes).
...
Este proyecto usa el modelo Qwen2-VL de Alibaba Cloud.
